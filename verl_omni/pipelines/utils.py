...
from .model_base import DiffusionI2IModelBase, DiffusionModelBase, WorldActionDiffusionModelBase
...
def prepare_model_inputs(
    ...
    if issubclass(model_cls, WorldActionDiffusionModelBase):
        model_inputs = model_cls.inject_action_inputs(model_inputs, micro_batch, step)
...
def normalize_reverse_step_output(output: Any) -> dict[str, torch.Tensor]:
    """Normalize legacy visual-only and joint WAM reverse-step outputs.

    Existing adapters return a four-tuple.  World-action adapters return a
    mapping so both modality densities from the same model forward can flow
    through the actor engine without another policy invocation.
    """
    visual_keys = ("log_probs", "prev_sample_mean", "std_dev_t", "sqrt_dt")
    action_keys = (
        "action_log_probs",
        "action_prev_sample_mean",
        "action_std_dev_t",
        "action_sqrt_dt",
    )
    ...
    if isinstance(output, Mapping):
        ...
        missing_visual = [key for key in visual_keys if key not in normalized]
        if missing_visual:
            raise KeyError(f"Reverse-step output is missing visual fields: {missing_visual}.")

        present_action = [key for key in action_keys if key in normalized]
        if present_action and len(present_action) != len(action_keys):
            missing_action = [key for key in action_keys if key not in normalized]
            raise KeyError(
                "World-action reverse-step output must provide the complete action density group; "
                f"missing {missing_action}."
            )
        return normalized

    if not isinstance(output, tuple | list) or len(output) != 4:
        raise TypeError("Reverse-step adapter output must be a four-tuple or a world-action mapping.")

    return dict(zip(visual_keys, output, strict=True))
