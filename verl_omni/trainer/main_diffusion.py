...
_WNM_RAY_ENV_PREFIXES = ("WAM_", "WNM_", "NCCL_")
_WNM_RAY_ENV_NAMES = {
    "ATTENTION_BACKEND",
    "BLIS_NUM_THREADS",
    "CFG_SCALE",
...
_SENSITIVE_ENV_SUFFIXES = ("_API_KEY", "_CREDENTIALS", "_PASSWORD", "_SECRET", "_TOKEN")


def _wnm_ray_runtime_env_vars(environ) -> dict[str, str]:
    """Select non-secret WNM runtime controls for every Ray worker.

    A pre-existing multi-node Ray cluster inherits the environment of each
    raylet, not environment variables exported later by the training wrapper.
    WNM uses those variables for sampler semantics, reward behavior and
    rollout optimizations, so silently dropping them changes both the model
    distribution and performance.  Keep the allow-list narrow instead of
    forwarding the full driver environment (which can contain credentials).
    """

    return {
        name: str(value)
        for name, value in environ.items()
        if (name in _WNM_RAY_ENV_NAMES or name.startswith(_WNM_RAY_ENV_PREFIXES))
        and not name.endswith(_SENSITIVE_ENV_SUFFIXES)
    }
...
def run_diffusion(config, task_runner_class=None) -> None:
    ...
    if not ray.is_initialized():
        ...
        driver_runtime_env = {
            "env_vars": _wnm_ray_runtime_env_vars(os.environ),
        }
        # Explicit config remains the final authority, while the current
        # driver environment supplies the WAM controls that a pre-existing
        # multi-node raylet cannot otherwise see.
        runtime_env = OmegaConf.merge(default_runtime_env, driver_runtime_env, runtime_env_kwargs)
        ray_init_kwargs = OmegaConf.create({**ray_init_kwargs, "runtime_env": runtime_env})
        propagated_env_names = sorted(driver_runtime_env["env_vars"])
        print(f"WNM Ray runtime env keys ({len(propagated_env_names)}): {propagated_env_names}")
...
class TaskRunner:
    ...
    def add_actor_rollout_worker(self, config):
        ...
        validation_reference_kl = config.algorithm.get("validation_reference_kl", None)
        validation_reference_kl_enabled = bool(
        ...
        if config.algorithm.sample_source == "offline":
            ...
            role = Role.Actor
        elif (need_reference_policy(config) or validation_reference_kl_enabled) and not ref_in_actor:
    ...
    def run(self, config):
        ...
        local_path = resolve_model_local_dir(
            ...
            local_files_only=config.actor_rollout_ref.model.get("local_files_only", False),
        )
        ...
        # Instantiate text/processor components only when the selected model
        # and dataset path need them.
        tokenizer = None
        processor = None
        if model_config.get("load_tokenizer", True):
            from verl.utils import hf_processor, hf_tokenizer

            trust_remote_code = config.data.get("trust_remote_code", False)
            tokenizer = hf_tokenizer(model_config.tokenizer_path, trust_remote_code=trust_remote_code)
