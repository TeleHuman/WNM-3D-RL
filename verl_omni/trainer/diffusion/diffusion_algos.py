...
def combine_visual_action_log_probs(
    *,
    log_prob: torch.Tensor,
    old_log_prob: torch.Tensor,
    action_log_prob: Optional[torch.Tensor] = None,
    old_action_log_prob: Optional[torch.Tensor] = None,
    visual_log_prob_weight: float = 1.0,
    action_log_prob_weight: float = 1.0,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Compose a weighted joint WAM transition likelihood without another model forward."""
    for name, value in (
    ...
    if (action_log_prob is None) != (old_action_log_prob is None):
        raise KeyError("Joint WAM likelihood requires both action_log_probs and old_action_log_probs.")
    if action_log_prob is None:
        if visual_log_prob_weight <= 0:
            raise ValueError("visual_log_prob_weight must be positive for a visual-only policy batch.")
        return visual_log_prob_weight * log_prob, visual_log_prob_weight * old_log_prob
    if old_log_prob.shape != log_prob.shape:
        raise ValueError(
            "Current and old visual log-probabilities must have identical shapes; "
            f"current={tuple(log_prob.shape)}, old={tuple(old_log_prob.shape)}."
        )
    action_log_prob = _aggregate_action_chunk_log_prob(action_log_prob, log_prob)
    old_action_log_prob = _aggregate_action_chunk_log_prob(old_action_log_prob, old_log_prob)
    if action_log_prob.shape != log_prob.shape or old_action_log_prob.shape != old_log_prob.shape:
        raise ValueError(
            "Visual and action log-probabilities must have identical per-transition shapes; "
            f"current_visual={tuple(log_prob.shape)}, current_action={tuple(action_log_prob.shape)}, "
            f"old_visual={tuple(old_log_prob.shape)}, old_action={tuple(old_action_log_prob.shape)}."
        )
    if visual_log_prob_weight == 0 and action_log_prob_weight == 0:
        raise ValueError("At least one joint WAM log-probability weight must be positive.")
    return (
        visual_log_prob_weight * log_prob + action_log_prob_weight * action_log_prob,
        visual_log_prob_weight * old_log_prob + action_log_prob_weight * old_action_log_prob,
    )


def _aggregate_action_chunk_log_prob(
    action_log_prob: torch.Tensor,
    visual_log_prob: torch.Tensor,
) -> torch.Tensor:
    """Reduce optional [.., C] temporal chunks with normalized loss weights."""

    if action_log_prob.shape == visual_log_prob.shape:
        return action_log_prob
    if action_log_prob.ndim != visual_log_prob.ndim + 1 or action_log_prob.shape[:-1] != visual_log_prob.shape:
        raise ValueError(
            "Action log-probability must match visual shape or append a chunk axis; "
            f"visual={tuple(visual_log_prob.shape)}, action={tuple(action_log_prob.shape)}."
        )
    weights = action_log_prob.new_tensor(normalized_action_chunk_weights(expected_chunks=action_log_prob.shape[-1]))
    return torch.sum(action_log_prob * weights, dim=-1)
...
class DiffusionLossFn(ABC):
    ...
    # Only losses that explicitly compose the visual/action policy density may
    # accept WAM action log-probabilities.
    supports_action_log_probs: bool = False
...
class DiffusionAdvantageEstimator(StrEnum):
...
class FlowGRPOLoss(DiffusionLossFn):
    ...
    supports_action_log_probs = True
    ...
    def compute_loss(
        ...
        *,
        ...
        action_log_prob: Optional[torch.Tensor] = None,
        old_action_log_prob: Optional[torch.Tensor] = None,
        action_advantages: Optional[torch.Tensor] = None,
        action_loss_weights: Optional[torch.Tensor] = None,
        action_terminal_safety_advantages: Optional[torch.Tensor] = None,
        action_stop_advantages: Optional[torch.Tensor] = None,
        action_stop_loss_weights: Optional[torch.Tensor] = None,
        action_collision_advantages: Optional[torch.Tensor] = None,
        action_collision_loss_weights: Optional[torch.Tensor] = None,
        ...
        visual_log_prob_weight = getattr(loss_cfg, "visual_log_prob_weight", 1.0)
        action_log_prob_weight = getattr(loss_cfg, "action_log_prob_weight", 1.0)
        action_stop_loss_weight = getattr(loss_cfg, "action_stop_loss_weight", 0.0)
        action_stop_use_chunk_weights = bool(getattr(loss_cfg, "action_stop_use_chunk_weights", False))
        action_collision_loss_weight = getattr(loss_cfg, "action_collision_loss_weight", 0.0)
        action_collision_use_chunk_weights = bool(getattr(loss_cfg, "action_collision_use_chunk_weights", False))
        terminal_safety_advantage_weight = float(os.environ.get("WAM_TERMINAL_SAFETY_ADVANTAGE_WEIGHT", "0.35"))
        if action_stop_loss_weight < 0:
            raise ValueError("action_stop_loss_weight must be non-negative")
        if action_collision_loss_weight < 0:
            raise ValueError("action_collision_loss_weight must be non-negative")
        if terminal_safety_advantage_weight < 0 or not math.isfinite(terminal_safety_advantage_weight):
            raise ValueError("WAM_TERMINAL_SAFETY_ADVANTAGE_WEIGHT must be finite and non-negative")
        visual_clip_ratio = getattr(loss_cfg, "visual_clip_ratio", None)
        action_clip_ratio = getattr(loss_cfg, "action_clip_ratio", None)
        if visual_clip_ratio is None:
            visual_clip_ratio = loss_cfg.clip_ratio
        if action_clip_ratio is None:
            action_clip_ratio = loss_cfg.clip_ratio
        ...
        if action_log_prob is not None:
            if action_advantages is None:
                raise KeyError("WAM DanceGRPO requires action_advantages for the action policy branch.")
            if old_action_log_prob is None:
                raise KeyError("WAM DanceGRPO requires old_action_log_prob for the action policy branch.")
            terminal_safety_required = os.environ.get(
                "WAM_TERMINAL_SAFETY_ADVANTAGE_ENABLED", "false"
            ).strip().lower() in {"1", "true", "yes", "on"}
            if terminal_safety_required and action_terminal_safety_advantages is None:
                raise KeyError(
                    "WAM terminal-safety advantage is enabled, but the actor "
                    "micro-batch has no action_terminal_safety_advantages. "
                    "Check trainer-to-engine transition field forwarding."
                )
            chunked_action = action_log_prob.ndim == log_prob.ndim + 1
            if chunked_action:
                if action_log_prob.shape[:-1] != log_prob.shape:
                    raise ValueError(
                        "Chunked action log-probability must append C to the visual shape; "
                        f"visual={tuple(log_prob.shape)}, action={tuple(action_log_prob.shape)}."
                    )
                if old_action_log_prob.shape != action_log_prob.shape:
                    raise ValueError(
                        "Current/old chunked action log-probability shapes differ: "
                        f"{tuple(action_log_prob.shape)} vs {tuple(old_action_log_prob.shape)}."
                    )
                if action_advantages.shape != action_log_prob.shape:
                    raise ValueError(
                        "Chunk action advantages must match action log-probabilities; "
                        f"advantages={tuple(action_advantages.shape)}, "
                        f"log_probs={tuple(action_log_prob.shape)}."
                    )
                if action_terminal_safety_advantages is not None and (
                    action_terminal_safety_advantages.shape != action_log_prob.shape[:-1]
                ):
                    raise ValueError(
                        "Terminal safety advantages must match the action "
                        "log-probability batch/timestep shape before the chunk "
                        "axis; safety="
                        f"{tuple(action_terminal_safety_advantages.shape)}, "
                        f"action={tuple(action_log_prob.shape)}."
                    )
                if action_stop_advantages is not None and (action_stop_advantages.shape != action_log_prob.shape):
                    raise ValueError(
                        "Chunk stop advantages must match action log-probabilities; "
                        f"advantages={tuple(action_stop_advantages.shape)}, "
                        f"log_probs={tuple(action_log_prob.shape)}."
                    )
                for weight_name, loss_weights in (
                    ("action_loss_weights", action_loss_weights),
                    ("action_stop_loss_weights", action_stop_loss_weights),
                    ("action_collision_loss_weights", action_collision_loss_weights),
                ):
                    if loss_weights is None:
                        continue
                    if loss_weights.shape != action_log_prob.shape:
                        raise ValueError(
                            f"{weight_name} must match chunked action log-probabilities; "
                            f"weights={tuple(loss_weights.shape)}, "
                            f"log_probs={tuple(action_log_prob.shape)}."
                        )
                    if not torch.isfinite(loss_weights).all() or bool((loss_weights < 0).any()):
                        raise ValueError(f"{weight_name} must contain finite non-negative values.")
                if action_collision_advantages is not None and (
                    action_collision_advantages.shape != action_log_prob.shape
                ):
                    raise ValueError(
                        "Chunk collision advantages must match action "
                        "log-probabilities; "
                        f"advantages={tuple(action_collision_advantages.shape)}, "
                        f"log_probs={tuple(action_log_prob.shape)}."
                    )
                chunk_weights = action_log_prob.new_tensor(
                    normalized_action_chunk_weights(expected_chunks=action_log_prob.shape[-1])
                )
            else:
                if (
                    action_log_prob.shape != log_prob.shape
                    or old_action_log_prob.shape != action_log_prob.shape
                    or action_advantages.shape != action_log_prob.shape
                ):
                    raise ValueError(
                        "Legacy action log-probabilities and advantages must match "
                        f"the visual shape {tuple(log_prob.shape)}; current_action="
                        f"{tuple(action_log_prob.shape)}, old_action="
                        f"{tuple(old_action_log_prob.shape)}, advantages="
                        f"{tuple(action_advantages.shape)}."
                    )
                chunk_weights = None
                if any(
                    value is not None
                    ...
                    raise ValueError("Active-only action loss weights require chunked action log-probabilities.")
                if action_stop_advantages is not None:
                    raise ValueError("Stop credit is supported only with chunked action log-probabilities.")
                if action_collision_advantages is not None:
                    raise ValueError("Collision credit is supported only with chunked action log-probabilities.")
                if action_terminal_safety_advantages is not None:
                    raise ValueError(
                        "Terminal safety advantage is supported only with chunked action log-probabilities."
                    )
            if action_stop_loss_weight > 0 and action_stop_advantages is None:
                raise KeyError("Positive action_stop_loss_weight requires action_stop_advantages.")
            if action_collision_loss_weight > 0 and action_collision_advantages is None:
                raise KeyError("Positive action_collision_loss_weight requires action_collision_advantages.")
            action_advantages = torch.clamp(
                action_advantages,
                -loss_cfg.adv_clip_max,
                loss_cfg.adv_clip_max,
            )
            if action_stop_advantages is not None:
                action_stop_advantages = torch.clamp(
                    action_stop_advantages,
                    -loss_cfg.adv_clip_max,
                    loss_cfg.adv_clip_max,
                )
            if action_collision_advantages is not None:
                action_collision_advantages = torch.clamp(
                    action_collision_advantages,
                    -loss_cfg.adv_clip_max,
                    loss_cfg.adv_clip_max,
                )
            if action_terminal_safety_advantages is not None:
                action_terminal_safety_advantages = torch.clamp(
                    action_terminal_safety_advantages,
                    -loss_cfg.adv_clip_max,
                    loss_cfg.adv_clip_max,
                )
            visual_log_ratio = log_prob - old_log_prob
            action_log_ratio = action_log_prob - old_action_log_prob
            visual_ratio = torch.exp(visual_log_ratio)
            action_ratio = torch.exp(action_log_ratio)

            visual_loss = torch.maximum(
                -advantages * visual_ratio,
                -advantages
                * torch.clamp(
                    visual_ratio,
                    1.0 - visual_clip_ratio,
                    1.0 + visual_clip_ratio,
                ),
            )
            # Fuse one trajectory-level safety advantage into the same
            # clipped PPO decision as navigation. Navigation retains its
            # active-only normalization; safety is deliberately broadcast to
            # all four jointly generated action chunks and is never masked by
            # collision truncation.
            effective_action_advantages = action_advantages
            action_weights_embedded = False
            if action_terminal_safety_advantages is not None:
                if action_loss_weights is not None:
                    effective_action_advantages = effective_action_advantages * action_loss_weights.detach()
                    action_weights_embedded = True
                effective_action_advantages = (
                    effective_action_advantages
                    + terminal_safety_advantage_weight * action_terminal_safety_advantages.unsqueeze(-1)
                )
            action_loss = torch.maximum(
                -effective_action_advantages * action_ratio,
                -effective_action_advantages
                * torch.clamp(
                    action_ratio,
                    1.0 - action_clip_ratio,
                    1.0 + action_clip_ratio,
                ),
            )
            if action_stop_advantages is not None:
                action_stop_loss = torch.maximum(
                    -action_stop_advantages * action_ratio,
                    -action_stop_advantages
                    * torch.clamp(
                        action_ratio,
                        1.0 - action_clip_ratio,
                ...
                action_stop_loss = None
            if action_collision_advantages is not None:
                action_collision_loss = torch.maximum(
                    -action_collision_advantages * action_ratio,
                    -action_collision_advantages
                    * torch.clamp(
                        action_ratio,
                        1.0 - action_clip_ratio,
                ...
                action_collision_loss = None
            if action_loss_weights is not None and not action_weights_embedded:
                action_loss = action_loss * action_loss_weights.detach()
            if action_stop_loss is not None and action_stop_loss_weights is not None:
                action_stop_loss = action_stop_loss * action_stop_loss_weights.detach()
            if action_collision_loss is not None and action_collision_loss_weights is not None:
                action_collision_loss = action_collision_loss * action_collision_loss_weights.detach()
            if rollout_is_weights is not None:
                detached_weights = rollout_is_weights.detach()
                visual_loss = visual_loss * detached_weights
                action_weights = detached_weights
                while action_weights.ndim < action_loss.ndim:
                    action_weights = action_weights.unsqueeze(-1)
                action_loss = action_loss * action_weights
                if action_stop_loss is not None:
                    action_stop_loss = action_stop_loss * action_weights
                if action_collision_loss is not None:
                    action_collision_loss = action_collision_loss * action_weights
            visual_pg_loss = visual_loss.mean()
            if chunked_action:
                action_chunk_losses = action_loss.mean(dim=tuple(range(action_loss.ndim - 1)))
                navigation_action_pg_loss = torch.sum(action_chunk_losses * chunk_weights)
                if action_stop_loss is not None:
                    action_stop_chunk_losses = action_stop_loss.mean(dim=tuple(range(action_stop_loss.ndim - 1)))
                    action_stop_pg_loss = (
                        torch.sum(action_stop_chunk_losses * chunk_weights)
                        if action_stop_use_chunk_weights
                        else action_stop_chunk_losses.mean()
                    )
                else:
                    action_stop_chunk_losses = None
                    action_stop_pg_loss = action_loss.new_zeros(())
                if action_collision_loss is not None:
                    action_collision_chunk_losses = action_collision_loss.mean(
                        dim=tuple(range(action_collision_loss.ndim - 1))
                    )
                    action_collision_pg_loss = (
                        torch.sum(action_collision_chunk_losses * chunk_weights)
                        if action_collision_use_chunk_weights
                        else action_collision_chunk_losses.mean()
                    )
                else:
                    action_collision_chunk_losses = None
                    action_collision_pg_loss = action_loss.new_zeros(())
                action_pg_loss = (
                    navigation_action_pg_loss
                    + action_stop_loss_weight * action_stop_pg_loss
                    + action_collision_loss_weight * action_collision_pg_loss
                )
            else:
                action_chunk_losses = None
                action_stop_chunk_losses = None
                action_stop_pg_loss = action_loss.new_zeros(())
                action_collision_chunk_losses = None
                action_collision_pg_loss = action_loss.new_zeros(())
                action_pg_loss = action_loss.mean()
            pg_loss = visual_log_prob_weight * visual_pg_loss + action_log_prob_weight * action_pg_loss
            with torch.no_grad():
                if chunked_action:
                    action_log_ratio_mean = torch.sum(
                        action_log_ratio.mean(dim=tuple(range(action_log_ratio.ndim - 1))) * chunk_weights
                    )
                    action_ratio_mean = torch.sum(
                        action_ratio.mean(dim=tuple(range(action_ratio.ndim - 1))) * chunk_weights
                    )
                    action_clipfrac_by_chunk = (
                        (torch.abs(action_ratio - 1.0) > action_clip_ratio)
                        .float()
                        .mean(dim=tuple(range(action_ratio.ndim - 1)))
                    )
                    action_clipfrac = torch.sum(action_clipfrac_by_chunk * chunk_weights)
                    action_advantage_mean = torch.sum(
                        action_advantages.mean(dim=tuple(range(action_advantages.ndim - 1))) * chunk_weights
                    )
                else:
                    action_log_ratio_mean = action_log_ratio.mean()
                    action_ratio_mean = action_ratio.mean()
                    action_clipfrac_by_chunk = None
                    action_clipfrac = (torch.abs(action_ratio - 1.0) > action_clip_ratio).float().mean()
                    action_advantage_mean = action_advantages.mean()
                metrics = {
                    "actor/visual_log_ratio_mean": visual_log_ratio.mean().item(),
                    "actor/action_log_ratio_mean": action_log_ratio_mean.item(),
                    "actor/visual_ratio_mean": visual_ratio.mean().item(),
                    "actor/action_ratio_mean": action_ratio_mean.item(),
                    "actor/visual_pg_clipfrac": (torch.abs(visual_ratio - 1.0) > visual_clip_ratio)
                    .float()
                    .mean()
                    .item(),
                    "actor/action_pg_clipfrac": action_clipfrac.item(),
                    "actor/visual_clip_ratio": visual_clip_ratio,
                    "actor/action_clip_ratio": action_clip_ratio,
                    "actor/visual_advantage_mean": advantages.mean().item(),
                    "actor/action_advantage_mean": action_advantage_mean.item(),
                    "actor/action_effective_advantage_abs_mean": (effective_action_advantages.abs().mean().item()),
                    "actor/visual_log_prob_weight": visual_log_prob_weight,
                    "actor/action_log_prob_weight": action_log_prob_weight,
                    "actor/action_stop_loss_weight": action_stop_loss_weight,
                    "actor/action_stop_use_chunk_weights": float(action_stop_use_chunk_weights),
                    "actor/action_stop_pg_loss": action_stop_pg_loss.item(),
                    "actor/action_active_normalization_enabled": float(action_loss_weights is not None),
                    "actor/action_stop_active_normalization_enabled": float(action_stop_loss_weights is not None),
                    "actor/action_backbone_grad_gain": float(os.environ.get("WAM_ACTION_BACKBONE_GRAD_GAIN", "4.0")),
                }
                if action_stop_advantages is not None:
                    metrics["actor/action_stop_advantage_mean"] = action_stop_advantages.mean().item()
                    metrics["actor/action_stop_advantage_abs_mean"] = action_stop_advantages.abs().mean().item()
                    metrics["actor/action_stop_pg_clipfrac"] = (
                        (torch.abs(action_ratio - 1.0) > action_clip_ratio).float().mean().item()
                    )
                if action_collision_advantages is not None:
                    metrics["actor/action_collision_loss_weight"] = action_collision_loss_weight
                    metrics["actor/action_collision_use_chunk_weights"] = float(action_collision_use_chunk_weights)
                    metrics["actor/action_collision_pg_loss"] = action_collision_pg_loss.item()
                    metrics["actor/action_collision_active_normalization_enabled"] = float(
                        action_collision_loss_weights is not None
                    )
                    metrics["actor/action_collision_advantage_mean"] = action_collision_advantages.mean().item()
                    metrics["actor/action_collision_advantage_abs_mean"] = (
                        action_collision_advantages.abs().mean().item()
                    )
                    metrics["actor/action_collision_pg_clipfrac"] = (
                        (torch.abs(action_ratio - 1.0) > action_clip_ratio).float().mean().item()
                    )
                if action_terminal_safety_advantages is not None:
                    metrics["actor/terminal_safety_advantage_weight"] = terminal_safety_advantage_weight
                    metrics["actor/terminal_safety_enabled"] = 1.0
                    metrics["actor/terminal_safety_advantage_mean"] = action_terminal_safety_advantages.mean().item()
                    metrics["actor/terminal_safety_advantage_abs_mean"] = (
                        action_terminal_safety_advantages.abs().mean().item()
                    )
                if chunked_action:
                    for index in range(action_log_prob.shape[-1]):
                        prefix = f"actor/action_chunk_{index}"
                        metrics[f"{prefix}_weight"] = chunk_weights[index].item()
                        metrics[f"{prefix}_loss"] = action_chunk_losses[index].item()
                        metrics[f"{prefix}_log_ratio_mean"] = action_log_ratio[..., index].mean().item()
                        metrics[f"{prefix}_ratio_mean"] = action_ratio[..., index].mean().item()
                        metrics[f"{prefix}_clipfrac"] = action_clipfrac_by_chunk[index].item()
                        metrics[f"{prefix}_advantage_mean"] = action_advantages[..., index].mean().item()
                        if action_loss_weights is not None:
                            chunk_active = action_loss_weights[..., index] > 0
                            metrics[f"{prefix}_active_fraction"] = chunk_active.float().mean().item()
                            metrics[f"{prefix}_active_normalization_gain"] = (
                                action_loss_weights[..., index].sum() / chunk_active.float().sum().clamp_min(1.0)
                            ).item()
                        if action_stop_chunk_losses is not None:
                            metrics[f"{prefix}_stop_loss"] = action_stop_chunk_losses[index].item()
                            metrics[f"{prefix}_stop_advantage_mean"] = action_stop_advantages[..., index].mean().item()
                            if action_stop_loss_weights is not None:
                                stop_active = action_stop_loss_weights[..., index] > 0
                                metrics[f"{prefix}_stop_active_fraction"] = stop_active.float().mean().item()
                                metrics[f"{prefix}_stop_active_normalization_gain"] = (
                                    action_stop_loss_weights[..., index].sum()
                                    / stop_active.float().sum().clamp_min(1.0)
                                ).item()
                        if action_collision_chunk_losses is not None:
                            metrics[f"{prefix}_collision_loss"] = action_collision_chunk_losses[index].item()
                            metrics[f"{prefix}_collision_advantage_mean"] = (
                                action_collision_advantages[..., index].mean().item()
                            )
            return pg_loss, metrics
        joint_log_prob, joint_old_log_prob = combine_visual_action_log_probs(
            log_prob=log_prob,
        ...
        log_ratio = joint_log_prob - joint_old_log_prob
        ...
        if action_log_prob is not None:
            pg_metrics.update(
                {
                    "actor/visual_log_ratio_mean": (log_prob - old_log_prob).mean().detach().item(),
                    "actor/action_log_ratio_mean": (action_log_prob - old_action_log_prob).mean().detach().item(),
                    "actor/visual_log_prob_weight": visual_log_prob_weight,
                    "actor/action_log_prob_weight": action_log_prob_weight,
                }
            )
    ...
    def __call__(
        ...
        *,
        ...
        loss, metrics = self.compute_loss(
            ...
            action_log_prob=model_output.get("action_log_probs", None),
            old_action_log_prob=data.get("old_action_log_probs", None),
            action_advantages=data.get("action_advantages", None),
            action_loss_weights=data.get("action_loss_weights", None),
            action_terminal_safety_advantages=data.get("action_terminal_safety_advantages", None),
            action_stop_advantages=data.get("action_stop_advantages", None),
            action_stop_loss_weights=data.get("action_stop_loss_weights", None),
            action_collision_advantages=data.get("action_collision_advantages", None),
            action_collision_loss_weights=data.get("action_collision_loss_weights", None),
        )
...
class KLLoss(DiffusionLossFn):
    ...
    def compute_loss(
        ...
        *,
        ...
        std_dev_t: torch.Tensor,
        action_prev_sample_mean: Optional[torch.Tensor] = None,
        ref_action_prev_sample_mean: Optional[torch.Tensor] = None,
        action_std_dev_t: Optional[torch.Tensor] = None,
    ) -> tuple[torch.Tensor, dict[str, Any]]:
        ...
        """
        visual_reduce_dims = tuple(range(1, prev_sample_mean.ndim))
        visual_kl = ((prev_sample_mean - ref_prev_sample_mean) ** 2).mean(dim=visual_reduce_dims, keepdim=True) / (
            2 * std_dev_t**2
        )
        total_kl = visual_kl.mean()
        metrics = {"actor/visual_kl_loss": total_kl.detach().item()}

        action_values = (action_prev_sample_mean, ref_action_prev_sample_mean, action_std_dev_t)
        if any(value is not None for value in action_values):
            if not all(value is not None for value in action_values):
                raise KeyError(
                    "Joint WAM KL requires action_prev_sample_mean, ref_action_prev_sample_mean, and action_std_dev_t."
                )
            action_reduce_dims = tuple(range(1, action_prev_sample_mean.ndim))
            action_kl = ((action_prev_sample_mean - ref_action_prev_sample_mean) ** 2).mean(
                dim=action_reduce_dims, keepdim=True
            ) / (2 * action_std_dev_t**2)
            action_kl = action_kl.mean()
            total_kl = total_kl + action_kl
            metrics["actor/action_kl_loss"] = action_kl.detach().item()

        metrics["actor/kl_loss"] = total_kl.detach().item()
    ...
    def __call__(
        ...
        *,
        ...
        kl_loss, metrics = self.compute_loss(
            ...
            std_dev_t=model_output["std_dev_t"],
            action_prev_sample_mean=model_output.get("action_prev_sample_mean", None),
            ref_action_prev_sample_mean=data.get("ref_action_prev_sample_mean", None),
            action_std_dev_t=model_output.get("action_std_dev_t", None),
        )
