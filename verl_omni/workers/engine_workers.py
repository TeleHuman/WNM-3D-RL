...
class TrainingWorker(Worker, DistProfilerExtension):
    ...
    def _postprocess_output(
        ...
        *,
        ...
        # Grad norm and learning rate are already reduced by the engine. Keep
        # them out of the generic DP all-gather, which expects per-microbatch
        # lists and would later try to flatten these scalars.
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(
        ...
        if omega_profiler_config.get("tool", None) in ["nsys", "torch", "torch_memory", "precision_debugger"]:
    ...
    def init_model(self):
        ...
        if "ref" in self.role:
            ...
            if not is_diffusion:
                # MTP is an autoregressive language-model feature. Diffusion
                # references do not use it and must not enter this generic
                # language-model compatibility path.
                object.__setattr__(
                    ref_config.model_config,
                    "mtp",
                    MtpConfig(enable=False),
                )
