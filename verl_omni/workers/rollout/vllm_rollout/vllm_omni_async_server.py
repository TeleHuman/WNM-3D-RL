...
WAM_AR_DIFFUSION_RUNNER = "verl_omni.pipelines.wnm_shared.ar_diffusion_runner_adapter.WNMDiffusionModelRunner"


def _install_wam_strict_batch_admission() -> None:
    """Wait for a full WAM request batch instead of vLLM's 50ms idle cutoff."""

    if os.getenv("WAM_ROLLOUT_REQUIRE_FULL_BATCH", "false").strip().lower() != "true":
        return
    from vllm_omni.diffusion.diffusion_engine import DiffusionEngine

    if getattr(DiffusionEngine, "_wam_strict_batch_admission_installed", False):
        return

    def wait_for_full_batch(self) -> None:
        if self.step_execution or not self.supports_request_batch:
            return
        max_wait_s = self.od_config.request_batch_max_wait_ms / 1000.0
        if max_wait_s <= 0 or self.scheduler.num_running_requests() > 0:
            return
        ...
        while not self.stop_event.is_set():
            ...
            if now >= deadline:
                logger.error(
                    "WAM strict request-batch admission timed out: waiting=%d expected=%d waited_ms=%.1f",
                    waiting,
                    max_batch,
                    (now - start) * 1000.0,
                )
                break
            self._cv.wait(timeout=min(deadline - now, 0.01))

        if not getattr(self, "_wam_logged_strict_batch_admission", False):
            self._wam_logged_strict_batch_admission = True
            logger.warning(
                "WAM strict request-batch admission released: waiting=%d expected=%d waited_ms=%.1f",
                self.scheduler.num_waiting_requests(),
                max_batch,
                (time.monotonic() - start) * 1000.0,
            )

    DiffusionEngine._wait_for_request_batch_admission_locked = wait_for_full_batch
    DiffusionEngine._wam_strict_batch_admission_installed = True


def _configure_wam_stage(engine_args: dict[str, Any]) -> None:
    """Apply the stage constraints required by the WAM rollout pipeline.

    The stock WNM deploy YAML enables ``step_cache``.  WAM uses a
    custom Dance-SDE pipeline that returns per-step log probabilities, so
    skipping DiT steps is both unsupported and mathematically incorrect.
    AR-Diffusion's paged KV path only supports one sequence per engine. WAM
    explicitly disables that cache, so the compatibility runner delegates to
    the generic diffusion batch runner and the custom pipeline can safely use
    its ``DiffusionRequestBatch`` implementation. Preserve the configured
    request-batch limit while making the remaining stage 0 constraints
    explicit.
    """
    stage_overrides = engine_args.get("stage_overrides")
    if isinstance(stage_overrides, str):
        stage_overrides = json.loads(stage_overrides)
    elif stage_overrides is None:
        stage_overrides = {}
    else:
        stage_overrides = dict(stage_overrides)

    tensor_parallel_size = int(engine_args.get("tensor_parallel_size") or 1)
    max_num_seqs = max(1, int(engine_args.get("max_num_seqs") or 1))
    request_batch_max_wait_ms = float(
        os.getenv("WAM_ROLLOUT_BATCH_WAIT_MS", engine_args.get("request_batch_max_wait_ms") or 0.0)
    )
    if request_batch_max_wait_ms < 0:
        raise ValueError(f"WAM_ROLLOUT_BATCH_WAIT_MS must be non-negative, got {request_batch_max_wait_ms}.")
    stage_zero = dict(stage_overrides.get("0") or {})
    stage_zero.update(
        {
            "cache_backend": "none",
            "cache_config": {},
            "devices": ",".join(str(rank) for rank in range(tensor_parallel_size)),
            "enforce_eager": True,
            "max_num_seqs": max_num_seqs,
            "request_batch_max_wait_ms": request_batch_max_wait_ms,
            "diffusion_model_runner_cls": WAM_AR_DIFFUSION_RUNNER,
        }
    )
    stage_overrides["0"] = stage_zero
    engine_args["stage_overrides"] = stage_overrides
    # This also covers the no-deploy-config fallback path.
...
class vLLMOmniHttpServer(vLLMHttpServer):
    ...
    async def run_server(self, args: argparse.Namespace):
        ...
        if self._ar_mode:
            ...
            if self.model_config.architecture in {"WNM2D", "WNM3D"}:
                _install_wam_strict_batch_admission()
                _configure_wam_stage(engine_args)
                wam_stage = engine_args["stage_overrides"]["0"]
                wam_max_num_seqs = int(wam_stage["max_num_seqs"])
                wam_batch_wait_ms = float(wam_stage["request_batch_max_wait_ms"])
                logger.warning(
                    "Configured %s stage 0 with cache disabled, "
                    "max_num_seqs=%d, request_batch_max_wait_ms=%.1f, and the generic request-batch runner",
        ...
        if attn_backend is not None:
            # vLLM-Omni's current stage configs already carry a structured
            # diffusion_attention_config. Passing the legacy shorthand beside
            # it is rejected as mutually exclusive, so express the rollout
            # choice through the authoritative structured field.
            engine_args.pop("diffusion_attention_backend", None)
            engine_args["diffusion_attention_config"] = {"default": {"backend": attn_backend}}
            logger.info("Setting diffusion_attention_config.default.backend=%s from rollout config", attn_backend)
    ...
    def _process_output(self, final_res, params, sampling_params: dict[str, Any]):
        ...
        actions = _maybe_unbatch(mm_output.get("actions"))
        if isinstance(actions, torch.Tensor):
            actions = actions.detach()

        if sampling_params.get("logprobs", False):
            log_probs = _maybe_unbatch(mm_output.get("all_log_probs"))
            action_log_probs = _maybe_unbatch(mm_output.get("action_log_probs"))
        else:
            log_probs = None
            action_log_probs = None

        has_action_trajectory = any(
            mm_output.get(key) is not None for key in ("all_action_latents", "all_action_timesteps")
        )
        if actions is not None:
            ...
            if sampling_params.get("logprobs", False):
                required_wam_fields += ("all_log_probs", "action_log_probs")
            missing_wam_fields = [key for key in required_wam_fields if mm_output.get(key) is None]
            if missing_wam_fields:
                raise KeyError(
                    "WAM custom_output must contain replayable visual/action trajectories"
                    + (" and log-probabilities" if sampling_params.get("logprobs", False) else "")
                    + f"; missing {missing_wam_fields}."
                )

            visual_latents = _maybe_unbatch(mm_output["all_latents"])
            visual_timesteps = _maybe_unbatch(mm_output["all_timesteps"])
            action_latents = _maybe_unbatch(mm_output["all_action_latents"])
            action_timesteps = _maybe_unbatch(mm_output["all_action_timesteps"])
            trajectory_lengths = {
                "visual_timesteps": len(visual_timesteps),
                "action_timesteps": len(action_timesteps),
                "visual_transitions": len(visual_latents) - 1,
                "action_transitions": len(action_latents) - 1,
            }
            if sampling_params.get("logprobs", False):
                trajectory_lengths.update(
                    visual_log_probs=len(log_probs),
                    action_log_probs=len(action_log_probs),
                )
            if len(set(trajectory_lengths.values())) != 1:
                raise ValueError(
                    "WAM custom_output trajectory lengths must describe the same policy steps; "
                    f"got {trajectory_lengths}."
                )
        elif has_action_trajectory or mm_output.get("action_log_probs") is not None:
            raise KeyError("WAM custom_output contains an action trajectory/log-probability but no actions.")

        reserved_output_keys = {"actions", "action_log_probs", "rollout_log_probs", "all_log_probs"}
        extra_fields = {k: _maybe_unbatch(v) for k, v in mm_output.items() if k not in reserved_output_keys}
        extra_fields["global_steps"] = self.global_steps
