...
from verl_omni.trainer.diffusion.diffusion_algos import combine_visual_action_log_probs, get_diffusion_loss_fn
...
def diffusion_loss(config: DiffusionActorConfig, model_output, data: TensorDict, dp_group=None):
    ...
    if "action_log_probs" in model_output and not loss_func.supports_action_log_probs:
        raise NotImplementedError(
            f"Diffusion loss `{loss_mode}` does not implement joint visual/action likelihoods. "
            "Use flow_grpo/dance_grpo or add an explicit WAM objective for this loss."
        )
    ...
    if "log_probs" in loss_func.required_model_output_keys:
        ...
        log_prob, old_log_prob = combine_visual_action_log_probs(
            log_prob=log_prob,
            old_log_prob=old_log_prob,
            action_log_prob=model_output.get("action_log_probs", None),
            old_action_log_prob=data.get("old_action_log_probs", None),
            visual_log_prob_weight=config.diffusion_loss.get("visual_log_prob_weight", 1.0),
            action_log_prob_weight=config.diffusion_loss.get("action_log_prob_weight", 1.0),
        )
